  ...
  void _showNowPlaying(BuildContext sheetContext) {
    ...
    if (channel == null) {
      ScaffoldMessenger.of(sheetContext).showSnackBar(
        const SnackBar(
          content: Text('Elige una estación para escuchar en vivo.'),
        ),
      );
      return;
    }
  ...
  Widget build(BuildContext context) {
    return const Card(
      child: Padding(
        ...
        child: Text('Aún no hay canales publicados.'),
      ),
    );
  }
}
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      errorMessage ?? (isPlaying ? 'En vivo' : 'Pausado'),
                      maxLines: 1,
                      overflow: TextOverflow.ellipsis,
                      style: TextStyle(
...
class _NowPlayingSheet extends StatelessWidget {
  ...
  final bool isBuffering;
  final Future<void> Function() onToggle;
  final Future<void> Function(Channel) onSelectChannel;
      ...
      child: Column(
        ...
        children: [
          ...
          if (channels.length > 1) ...[
            ...
            ...channels
                .where((item) => item.slug != channel.slug)
                .map(
                  (item) => ListTile(
...
class _ParticipatePage extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return const SafeArea(child: SongRequestPage());
  }
}
